"""Deterministic, headless policy evaluation for the v1 balance environment.

Examples (run from RL/v1):
    python eval1.py                         # quick check: best run 10, seeds 0..19
    python eval1.py --episodes 300          # fuller fixed-seed audit
    python eval1.py --run 10 --final        # evaluate final rather than best
    python eval1.py --run 10 11 --episodes 300  # compare on identical seeds
    python eval1.py --seed-start 20000      # use a different fixed seed set
"""
...
def main():
    ...
    parser.add_argument("--run", type=int, nargs="+", default=[10])
